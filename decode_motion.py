# from https://github.com/Svito-zar/speech-driven-hand-gesture-generation-demo/blob/master/decode_motion.py
...
class DAE:
    ...
    def __init__(self, shape, sess, args):
        ...
        with sess.graph.as_default():
            ...
            with tf.variable_scope("AE_Variables"):
                ...
                self._output, self._encode, self._decode = self.construct_graph(
                    self._input_, args.dropout)
    ...
    def construct_graph(self, input_seq_pl, dropout):
        ...
        curr_layer = tf.reshape(
            network_input, [self.batch_size, self.__shape[0]])
...
def smoothing(motion):
    ...
    smoothed = [savgol_filter(motion[:, i], 9, 3)
